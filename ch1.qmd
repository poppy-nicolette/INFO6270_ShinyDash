...
These are also called widgets, but there are other forms in which text or numbers can be entered. There is a basic format to the inputs. First there is the input type, in this case a *sliderInput*. In the first position inside is the inputID parameter, followed by the label parameter. These two will be consistent with all inputs. After the inputID and the label, arguments that may be unique to each follow. In this case, there are arguments for *value* *min*, and *max*. In general, all inputs, (including widgets) keep this order of *inputID, label, arguments*.
