...
Widgets are also a type of input, but there are other forms in which text or numbers can be entered. There is a basic format to the inputs, just like widgets. First there is the input type, in this case a *textAreaInput*. In the first position inside is the inputID parameter, followed by the label parameter. These two will be consistent with all inputs. After the inputID and the label, arguments that may be unique to each follow. In this case, there is an argument for *the number of rows*. In general, all inputs, (including widgets) keep this order of *inputID, label, arguments*.
...
We'll cover more of these in the widgets section.
